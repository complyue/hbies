...
function sendBufs(ws, bufs, resolve, reject, idx = 0) {
  try {
    while (true) {
      var sendMore = ws.sendBinary(bufs[idx])
      ...
      if (!sendMore) {
        ws.socket.once('drain', ()=> {
          sendBufs(ws, bufs, resolve, reject, idx)
        })
        return
      }
    }
...
class SWSHBIC extends NodeHBIC {
  ...
  _sendText(codeObj, wireDir = '') {
    return new P((resolve, reject)=> {
      try {
        ...
        resolve(ws.sendText(PACK_BEGIN + wireDir + PACK_END + payload))
      } catch (err) {
        reject(err)
      }
    })
  ...
  _sendData(bufs) {
    ...
    return new P((resolve, reject)=> {
      sendBufs(this.transport, bufs, resolve, reject)
    })
  }
  ...
  wire(ws) {
    ...
    ws.on('error', (err)=> {
      ...
      return this.constructor.handleWireErr(this, err, ws)
    })
    if (!this.sendOnly) {
      ws.on('close', ()=> {
      ...
      ws.on('binary', (inStream)=> {
        if (!this._dataSink) {
          return this.constructor.handleWireErr(this, new Error('unexpected binary frame'), ws)
        }
      ...
      ws.on('text', (payload)=> {
        if (!payload.startsWith(PACK_BEGIN)) {
          return this.constructor.handleWireErr(this, new Error('HBI packet from [' + this.addr + '] malformed, invalid PACK_BEGIN: ' + payload.substr(0, PACK_BEGIN.length)), ws)
        }
        var pePos = payload.substr(0, PACK_HEADER_MAX).indexOf(PACK_END, PACK_BEGIN.length)
        if (!pePos) {
          return this.constructor.handleWireErr(this, new Error('HBI packet from [' + this.addr + '] malformed, no PACK_END found in header: ' + payload.substr(0, PACK_HEADER_MAX)), ws)
        }
