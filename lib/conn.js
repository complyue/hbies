...
class AbstractHBIC extends EventEmitter {
   ...
   * @param g a generator
   */
  static promise(g, restoreCtx = null) {
        ...
        while (true) {
          ...
          if (awv) {
            ...
            if ('function' === typeof awv.next && 'function' === typeof awv.throw) {
              this.constructor.promise(awv, restoreCtx).then((rv)=> {
                setImmediate(itrun, rv)
              }, (err)=> {
                // if the awaited promise rejects, propagate the err to generator
                try {
  ...
  constructor(context, addr, {
    ...
    lowWaterMarkSend = 6 * 1024 * 1024, highWaterMarkSend = 20 * 1024 * 1024,
    lowWaterMarkRecv = 6 * 1024 * 1024, highWaterMarkRecv = 20 * 1024 * 1024
  }={}) {
    super()
    ...
    this._checkDrain = ()=> {
      ...
      var buffered = this._getSendWaterPos()
    ...
    this._recv_buffer = new BufferList()
    this._dataSink = null
    ...
    this._corunMutex = ()=> {
      if (this._corunning) {
        ...
        return new P((resolve, reject)=> {
          this._coTaskQueue.push([resolve, reject])
   ...
   */
  _getSendWaterPos() {
    throw new Error('HBIC subclass [' + this.constructor.name + '] did not override _getSendWaterPos!')
  }

  /**
   * @returns the amount of data currently cached for receiving, in bytes
   */
  _getRecvWaterPos() {
    throw new Error('HBIC subclass [' + this.constructor.name + '] did not override _getRecvWaterPos!')
  }
  ...
  _recvData(bufs) {
    ...
    return new P((resolve, reject)=> {
      ...
      const dataSink = (chunk)=> {
        try {
          chunk = this.constructor.mapBuffer(chunk)
          while (true) {
            if (buf) {
              ...
              var consumed = this.constructor.copyBuffer(chunk, buf, pos)
              pos += consumed
              if (pos >= buf.length) {
  ...
  coRecvObj() {
    ...
    // no object from buffer available for now, queue as a waiter, return the wrapping promise
    return new P((resolve, reject)=> {
  ...
  _beginOffload(sink) {
    ...
    if (this._recv_buffer.length > 0) {
      ...
      do {
        var chunk = this._recv_buffer.shift()
        if (!chunk) break // no more data in buffer
        sink(chunk) // mangics can happen inside this call
  ...
  _endOffload(readAhead, originalSink = null) {
    ...
    if (readAhead) {
      // prepend to buffer
      this._recv_buffer.unshift(readAhead)
    ...
    while (true) {
      ...
      if (!this._corunning) {
        ...
        while (true) {
          ...
          if (this._getRecvWaterPos() <= this.lowWaterMarkRecv) {
            this._resumeRecv()
          }
          var got = this._landOne()
          if (!got) {
      ...
      if (this._corunning) {
        ...
        var bufferedAmount = this._getRecvWaterPos()
  ...
  /**
   * Run a generator as a coroutine, while having this HBIC track its execution.
   *
   ...
   * pressure as pending amount raises beyond `highWaterMark`. The coroutine is expected to read packet landing results
   * and binary data through `coRecvObj` and `coRecvData` by yielding the returned promises of these method calls, the
   * settled result - either value sent back to the yield statement or exception thrown from it - is sent to the
   * generator function, thus make it in fact `coroutine`. When the coroutine terminates either by returning or letting
   * through unhandled error, this HBIC will launch next coroutine if in queue, or resume hosting mode.
   *
   * @param g a generator
   */
  corun(g, rejectionHandler = null) {
    if (!g || 'function' !== typeof g.next || 'function' !== typeof g.throw) {
    ...
    if (!rejectionHandler) {
      rejectionHandler = (err)=> {
        this.handleLandingErr(err)
      }
    }

    // use send+corun mutex to prevent interference with other sendings or coros
    return P.using(this._sendMutex(), this._corunMutex(), ()=> {
        return this.constructor.promise(g, ()=> {
          // set $peer$ every time progressing the coro
          this.context.$peer$ = this
