...
class AbstractHBIC extends EventEmitter {
  ...
  constructor(context, addr, {
    ...
    lowWaterMarkerSend = 2 * 1024 * 1024, highWaterMarkSend = 12 * 1024 * 1024,
    lowWaterMarkerRecv = 2 * 1024 * 1024, highWaterMarkRecv = 12 * 1024 * 1024
  }={}) {
    super()
   ...
   * to flow control
   */
  coSendCode(code, wireDir) {
    if (!this._corunning) {
      throw new Error('not in corun mode')
    }
    return new P(this._sendCodeTask(code, wireDir))
  }
  ...
  sendCode(code, wireDir) {
    ...
    return this._sendMutex.orderly(this._sendCodeTask(code, wireDir))
  }
  ...
  sendData(bufs) {
    // use mutex to prevent interference
    return this._sendMutex.orderly(this._sendDataTask(bufs))
  }
  ...
  send(code, bufs) {
    ...
    return this._sendMutex.orderly((resolve, reject)=> {
      new P(this._sendCodeTask(code)).then(()=>new P(this._sendDataTask(bufs))).then(resolve, reject)
    })
  }
  ...
  recvData(bufs) {
    if (this._corunning) {
      throw new Error('not in hosting mode')
    }
    return this._recvData(bufs)
  }

  _beginOffload(sink) {
    throw new Error('HBIC subclass [' + this.constructor.name + '] did not override _beginOffload!')
  }

  _endOffload(readAhead, originalSink = null) {
    throw new Error('HBIC subclass [' + this.constructor.name + '] did not override _endOffload!')
  }
  ...
  coRecvObj() {
    ...
    while (true) {
      ...
      var [got,err,obj] = this._landOne()
      if (got) {
        if (this._recvObjWaiters.length > 0) {
          // resolve previous waiters first
          var objWaiter = peer._recvObjWaiters.shift()
          if (one.err) {
            // reject
            objWaiter[1](one.err)
          } else {
  ...
  _readWire() {
    ...
    if (this._corunning) {
      ...
      while (true) {
        ...
        if (this._corunning) {
          // switched to corun mode during landing, stop reading wire,
          // let subsequent incoming data to trigger hwm back pressure
          return
        }
      }
    ...
    // in corun mode, consume buffer only due to waiters, and make flow ctrl effective

    // resolve as many obj waiters as possible from buffered data
    ...
    if (!socket || socket.destroyed)
      return
    var bufferedAmount = socket._readableState.length
    if (bufferedAmount > this.highWaterMarkRecv) {
      this._pauseRecv()
    } else if (bufferedAmount <= this.lowWaterMarkerRecv) {
      this._resumeRecv()
    }
  ...
  corun(g) {
    ...
    if (this._corunning) {
      // is running coroutine, queue it for later execution
      return new P((resolve, reject)=> {
        this._coTaskQueue.push([g, resolve, reject])
      })
    }

    // start running this coro, switch to corun mode
    this._corunning = true
    this._readWire()

    // this implements the finally() callback to start next coro upon settlement of the previous
    const finishCo = ()=> {
      var nextCo = this._coTaskQueue.unshift()
      if (nextCo) {
        // got next coro to run
        var [g,resolve,reject]=nextCo
        this.constructor.promise(g).then(resolve, reject).finally(finishCo)
      } else {
    ...
    return this.constructor.promise(g).finally(finishCo)
  }
  ...
  cancelPendingCoTasks(err) {
    ...
    // call reject handler
    for (var [t,resolve,reject] of this._coTaskQueue.splice(0, this._coTaskQueue.length)) {
      rejectc(err)
    }
  }
