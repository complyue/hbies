...
function handleLandingErr(peer, socket, err, ...args) {
  // try emit as local landing err
  if (peer.emit(CONSTS.LANDING_ERR_EVENT, err, socket, ...args)) {
    // landing errors are locally listened, and
    if (socket.destroyed || peer.disconnecting) {
...
class HBIC extends EventEmitter {
    ...
    this._data_handler = function (chunk) {
      ...
      chunk_processing: while (chunkPos < chunk.length) {
        ...
        if (wireDir) {
          ...
          if (landingErr) {
            if (!handleLandingErr(peer, this, landingErr, payload)) {
              // not recoverable, assume unwired by handlLandingErr
              return
            }
          }
        }
  ...
  sendPeerError(err, cb) {
    ...
    return this.sendWire(`this.handlePeerErr(${JSON.stringify(err.message)},${JSON.stringify(err.stack)})`, cb)
  }
