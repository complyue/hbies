...
class HBIC extends EventEmitter {
  ...
  constructor(context) {
    ...
    const peer = this
    // define socket event listeners as properties, allow them to be explicitly removed if necessary
    ...
    this._err_handler = function (err) {
      ...
      peer.emit(CONSTS.WIRE_ERR_EVENT, err, this)
    }
    ...
    this._data_handler = function (chunk) {
      ...
      chunk_processing: while (chunkPos < chunk.length) {
        ...
        if (!peer._wireBuf.pdBuf) { // filling packet header
          ...
          if (hdrEnd < 0) {
            if (peer._wireBuf.phPos >= peer._wireBuf.phBuf.length) {
              var err = new Error('HBI packet header from [' + this.remoteAddress + ':' + this.remotePort + '] overflow, no PACK_END found in first ' + PACK_HEADER_MAX + ' bytes: ' + peer._wireBuf.phBuf.toString('utf8', 0, peer._wireBuf.phPos))
              peer.emit(CONSTS.WIRE_ERR_EVENT, err, socket)
              peer._unwire(socket)
            }
          ...
          if (peer._wireBuf.phBuf[0] !== PACK_BEGIN) {
            var err = new Error('HBI packet header from [' + this.remoteAddress + ':' + this.remotePort + '] malformed, invalid PACK_BEGIN: ' + peer._wireBuf.phBuf.toString('utf8', 0, peer._wireBuf.phPos))
            peer.emit(CONSTS.WIRE_ERR_EVENT, err, socket)
            peer._unwire(socket)
            return
          }
          ...
          if (plemp < 0) {
            var err = new Error('HBI packet header from [' + this.remoteAddress + ':' + this.remotePort + '] malformed, no length end marker (' + PACK_LEN_END + '): ' + peer._wireBuf.phBuf.toString('utf8', 0, peer._wireBuf.phPos))
            peer.emit(CONSTS.WIRE_ERR_EVENT, err, socket)
            peer._unwire(socket)
            return
          }
          ...
          if (!isFinite(pl)) {
            var err = new Error('HBI packet header from [' + this.remoteAddress + ':' + this.remotePort + '] malformed, invalid packet length: ' + peer._wireBuf.phBuf.toString('utf8', 0, peer._wireBuf.phPos))
            peer.emit(CONSTS.WIRE_ERR_EVENT, err, socket)
            peer._unwire(socket)
            return
          }
        ...
        if (wireDir) {
          ...
          if (wireErr) {
            peer.emit(CONSTS.WIRE_ERR_EVENT, wireErr, socket)
            this.pause()
            peer._unwire(socket)
            return
          }
          ...
          try {
            ...
            if (peer.emit(CONSTS.LANDING_ERR_EVENT, err, payload, socket)) {
  ...
  resumeData(sink, readAhead) {
    if (this._dataSink !== sink) {
      throw new Error('unpaired sinkData/resumeData call')
    }
    this._dataSink = null
    this._data_handler(readAhead)
  }

  wire(socket, readAhead) {
    if (this.socket) {
  ...
  send(codeObj, cb) {
    this._send(codeObj, cb)
  }

  sendWire(codeObj, cb) {
    this._send(codeObj, cb, WIRE_DIRECTIVE)
  }
  ...
  sendPeerError(err, cb) {
    this.sendWire('this.emit("' + CONSTS.PEER_ERR_EVENT + '",' + JSON.stringify(err + '') + ')', cb)
  }
