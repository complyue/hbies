...
function sendBufs(socket, bufs, cb, idx = 0) {
  var sendMore = true
  while (idx < bufs.length) {
    if (cb && bufs.length - 1 === idx)
      sendMore = socket.write(bufs[idx], cb)
    else
      sendMore = socket.write(bufs[idx])
    idx++
    if (!sendMore) {
      if (idx < bufs.length) {
        socket.once('drain', ()=> {
          sendBufs(socket, bufs, cb, idx)
        })
      }
      return false
    }
...
class HBIC extends NodeHBIC {
  ...
  constructor(...args) {
    ...
    this._err_handler = function (err) {
      ...
      return peer.constructor.handleWireErr(peer, this, err)
    }
