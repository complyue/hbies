...
class HBIC extends NodeHBIC {
  ...
  /**
   * pump/land one packet from this._recv_buffer
   */
  _landOne() {
    var chunk
    while (chunk = this._recv_buffer.shift()) {
      if (chunk.length <= 0)
      ...
      while (true) {
        if (!this._wireBuf.pdBuf) { // filling packet header
          ...
          if (hdrEnd < 0) {
            ...
            if (chunk.length - chunkPos + this._wireBuf.phPos >= PACK_HEADER_MAX) {
              this.handleWireErr(new Error('HBI packet header from ' + this.netInfo
                + ' overflow, no PACK_END found in first ' + (chunk.length - chunkPos + this._wireBuf.phPos)
                + ' bytes: ['
                + this._wireBuf.phBuf.toString('utf-8', 0, this._wireBuf.phPos) + chunk.toString('utf-8', chunkPos)
                + ']'))
          ...
          if (this._wireBuf.phBuf[0] !== PACK_BEGIN) {
            this.handleWireErr(new Error('HBI packet header from ' + this.netInfo
              + ' malformed, invalid PACK_BEGIN: [' + this._wireBuf.phBuf.toString('utf-8', 0, this._wireBuf.phPos)
              + ']'))
            // this is not recoverable, return nothing from here, err should have been propagated
            return
          }
          var hdrStr = this._wireBuf.phBuf.toString('utf-8', 1, this._wireBuf.phPos - 1)
          var plemp = hdrStr.indexOf(PACK_LEN_END)
          if (plemp < 0) {
            this.handleWireErr(new Error('HBI packet header from ' + this.netInfo
              + ' malformed, no length end marker (' + PACK_LEN_END + '): ['
              + this._wireBuf.phBuf.toString('utf-8', 0, this._wireBuf.phPos) + ']'))
            // this is not recoverable, return nothing from here, err should have been propagated
            return
          }
          ...
          if (!isFinite(pl)) {
            this.handleWireErr(new Error('HBI packet header from ' + this.netInfo
              + ' malformed, invalid packet length: [' + this._wireBuf.phBuf.toString('utf-8', 0, this._wireBuf.phPos)
              + ']'))
            // this is not recoverable, return nothing from here, err should have been propagated
            return
          }
        ...
        if (chunkPos < chunk.length) {
          this._recv_buffer.unshift(chunk.slice(chunkPos))
        }
  ...
  _getRecvWaterPos() {
    return this._recv_buffer.length
  }
