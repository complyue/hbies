...
class HBIC extends NodeHBIC {
  ...
  constructor(...args) {
    ...
    this._data_handler = function (chunk) {
      ...
      // if data sink is planted, regardless of hosting or corun mode, simply forward data to it
      if (peer._dataSink) {
    ...
    while (chunk = this._buffer.shift()) {
      ...
      while (true) {
        if (!this._wireBuf.pdBuf) { // filling packet header
          ...
          if (hdrEnd < 0) {
            ...
            if (chunk.length - chunkPos + this._wireBuf.phPos >= PACK_HEADER_MAX) {
              this.constructor.handleWireErr(this, new Error('HBI packet header from ' + this.netInfo
        ...
        if (this._wireBuf.pdPos < this._wireBuf.pdBuf.length) {
          var byts = chunk.copy(this._wireBuf.pdBuf, this._wireBuf.pdPos, chunkPos, Math.min(chunk.length, chunkPos + this._wireBuf.pdBuf.length - this._wireBuf.pdPos))
        ...
        var payload = this._wireBuf.pdBuf.toString('utf8')
        // reset packet bufs
        ...
        if (chunkPos < chunk.length) {
          // if this chunk is not fully consumed, prepend back to buffer
  ...
  _endOffload(readAhead, originalSink = null) {
    ...
    if (readAhead) {
      if (this._buffer.length > 0) {
        // buffered data not thoroughly consumed, should prepend to those
        this._buffer.unshift(readAhead)
      } else {
        // no buffered data, handle as new data coming
