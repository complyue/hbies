...
function sendBufs(socket, bufs, resolve, reject, idx = 0) {
  try {
    while (true) {
      var sendMore = socket.write(bufs[idx])
      ...
      if (!sendMore) {
        socket.once('drain', ()=> {
          sendBufs(socket, bufs, resolve, reject, idx)
        })
        return
      }
    }
